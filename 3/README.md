...
만약 a 값이 10 이상 또는 초과일경우 while 문의 비교문의 bool값이 False임으로 실행되지 않는다.<Br>
위 코드를 실행해보면 0 ~ 9 까지는 10 미만임을 성립함으로 "10 미만" 이라는 문장이 10번 실행될 것이다.<br>
조금 더 쉽게 표로 알아보자면<br>
| 변수 a | 비교문  | bool 값 | 수행할 문장                        | while 문 상태 |
| ------ | ------- | -------------- | ---------------------------------- | ------------- |
...
이렇게 볼 수 있다.  
bool 부분은 앞서 설명한 비교 연산자를 이용한 것이므로 당연하게 문자도 집어넣을 수 있다. 
```py3
we = "logic"
